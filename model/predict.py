...
def predict_sessions(model, full_data, keys, content_dim, threshold, output_filename,
    ...
    for step, batch in enumerate(loader):
        ...
        padded_input = Variable(torch.Tensor(
            input_padded), requires_grad=False)
        padded_label = Variable(torch.Tensor(
            label_padded), requires_grad=False)
        model.init_hidden()
        y_pred = model(padded_input, seq_lens)
        threshold_output, correct_ones = find_max_predictions(
            y_pred, padded_label, threshold)
        sample_writer = writer_sample_output(
                                student, sessions, padded_input,
                                threshold_output, padded_label, correct_ones,
                                exercise_to_index_map, include_correct)
        output_writer.append(sample_writer)
...
def writer_sample_output(student, sessions, padded_input,
                         threshold_output, padded_label, correct_ones,
                         exercise_to_index_map, include_correct):
    '''
    ...
    output = write_student_sample(student, sessions, stud_input,
                         actual, prediction, correct,
                         index_to_exercise_map, include_correct)
    return output


def write_student_sample(student, sessions, stud_input,
    ...
    for i, label in enumerate(actual):
        ...
        new_str = student_session + '\t' + \
            str(readable_input) + '\t' + \
            str(readable_output) + '\t' + \
            str(readable_label) + '\n'
        sample_writer.append(str(new_str))
...
def run_inference():
    ...
    return predict_sessions(model, full_data, keys, content_dim, threshold,
